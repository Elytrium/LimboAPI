...
public class MapPalette {
  ...
  private static final byte[] MAIN_BUFFER = MapPalette.readBuffer("/mappings/colors_main_map");
  ...
  static {
    for (MapVersion version : MapVersion.values()) {
      MapPalette.REMAP_BUFFERS.put(version, readBuffer("/mappings/colors_" + version.toString().toLowerCase(Locale.ROOT) + "_map"));
    }
  }
